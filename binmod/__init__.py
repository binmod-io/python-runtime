...
class ModuleEnv(BaseModel):
    """
    WebAssembly module environment.
    """
...
class FnError(Exception):
    """
    A function error returned from a module function.
    """
    message: str
    type: str
...
class FnResultData(BaseModel, extra="allow"):
    """
    A function serialized result data returned from a module function.
    """
...
class FnResultError(BaseModel, extra="allow"):
    """
    A function serialized result error returned from a module function.
    """
...
class FnResult(RootModel):
    """
    A function result returned from a module function.
    """
    root: Annotated[
        Union[
...
class FnInput(BaseModel):
    """
    A function input to be passed to a module function.
    """
...
class Module:
    ...
    def _register_host_fns(
        ...
        # We always define host_alloc and host_dealloc funcs
        self._linker.define(
            store=self._store,
            module=self._namespace,
            name="host_alloc",
            item=Func(
                ...
                func=lambda size: self._guest_alloc(self._store, size),  # type: ignore[misc]
                access_caller=False
            )
        )
        self._linker.define(
            ...
            item=Func(
                ...
                func=lambda ptr, size: self._guest_dealloc(self._store, ptr, size),  # type: ignore[misc]
                access_caller=False
            )
        )
